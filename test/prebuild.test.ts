...
beforeAll(async () => {
  ...
  tar = join(dir, 'ffmpeg-native-v9.9.9-darwin-arm64.tar.gz');
...
describe('computePins', () => {
  it('maps each tarball to its sha256, keyed by <platform>-<arch>', async () => {
    const muslTar = join(dir, 'ffmpeg-native-v9.9.9-linux-musl-x64.tar.gz');
  ...
  it('ignores non-tarball files', async () => {
    writeFileSync(join(dir, 'ffmpeg-native-v9.9.9-notes.txt'), 'ignore me');
    const pins = await computePins(dir, '9.9.9');
