 ...
 *
 * Prebuilt tarball naming: ffmpeg-native-v{version}-{platform}-{arch}.tar.gz
...
const tarName = `ffmpeg-native-v${version}-${platformKey}-${arch}.tar.gz`;

// hardcoded origin ensures the URL can never point to an attacker-controlled host
const releaseOrigin = 'https://github.com';
const releasePath = `/xonaman/nodejs-ffmpeg-native/releases/download/v${version}/${tarName}`;
...
async function tryDownload() {
  ...
  try {
    ...
    console.log('Prebuilt archive extracted but ffmpeg.node not found, will compile from source.');
    return false;
  } catch (err) {
...
const nodeFile = join(outDir, 'ffmpeg.node');
if (existsSync(nodeFile)) {
  console.log('ffmpeg.node already exists, skipping install.');
  process.exit(0);
}
